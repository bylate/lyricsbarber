...
segments, _ = model.transcribe(vocal, word_timestamps=True)
tstamp_of_all_words = []
tstamp_of_bad_words = []
finishtime=0
cfnow=0
for segment in segments:
    for word in segment.words:
        ...
        tstamp_of_all_words.append([word.start, word.end,word.word])
        if (predict_prob([word.word])) >= 0.75:
            tstamp_of_bad_words.append([word.start,word.end])
        finishtime=word.end
